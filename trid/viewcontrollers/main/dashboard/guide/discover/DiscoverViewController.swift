...
class DiscoverViewController: DashboardBaseViewController {
   ...
   @IBOutlet weak var groupImages: ImageSlideshow!
    ...
    override func viewDidLoad() {
        ...
        getGroupImages()
       // image.image = UIImage(named: "img-default")
...
//        // Position
    ...
    func getGroupImages()  {
        HTTP.GET("\(ApiRouts.Web)/api/groups/\((currentGroup?.id)!)/images", parameters: []) { response in
            ...
            do {
                ...
                DispatchQueue.main.sync {
                    self.groupImagess = images
                    if self.groupImagess.count == 0 {
                        self.groupImages.setImageInputs([
                            ImageSource(image: UIImage(named: "img-default")!)])
                    }else {
                        
                        var image_path: String = ""
                        for image in self.groupImagess {
                            if image.path !=  nil {
