...
//        labelName.text = Localized.AppFullName
        ...
        self.guestBtn?.layer.cornerRadius = radius
...
//        }
    ...
    */
    ...
    func makeBackgroundSlideshow() {
        ...
        // source
        let photos = ["welcome_bg_1", "welcome_bg_2", "welcome_bg_3"]
...
//            UIView.animate(withDuration: 0.2, animations: {
    ...
    fileprivate func facebookLogin() {
        print("Im here in actionSignInWithFacebook")
        makeAllButtonEnabled(false)
    ...
    @IBAction func actionSignInWithFacebook(_ sender: Any) {
       // facebookLogin()
        FBSDKLoginManager().logIn(withReadPermissions: ["email"], from: self) { (result, err) in
            ...
            self.loginFacebookFun()
        }
    }
    func loginFacebookFun() {
        print("No Error")
        ...
        let _ = request?.start(completionHandler: { (connection, result, error) in
            ...
            let userDetails:SignInInfo = SignInInfo()
            guard let userInfo = result as? [String: Any] else { return }
            print("user indfo \(userInfo)")
            if let imageURL = ((userInfo["picture"] as? [String: Any])?["data"] as? [String: Any])?["url"] as? String {
             //   let url = URL(string: imageURL)
                userDetails.image_path = imageURL
                self.setToUserDefaults(value: imageURL, key: "profile_image")
                
            }
            self.setToUserDefaults(value: true, key: "isLogged")
            self.setToUserDefaults(value: userInfo["first_name"] != nil ? userInfo["first_name"] as? String : "" , key: "first_name")
            self.setToUserDefaults(value: userInfo["last_name"] != nil ? userInfo["last_name"] as? String : "", key: "last_name")

            userDetails.first_name = userInfo["first_name"] != nil ? userInfo["first_name"] as? String : ""
            userDetails.last_name = userInfo["last_name"] != nil ? userInfo["last_name"] as? String : ""
            
           self.dismiss(animated: true, completion: nil)
            NotificationCenter.default.post(name: NotificationKey.signedIn, object: userDetails)
