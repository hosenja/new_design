...
class AppState : NSObject{
    ...
    static var isSelecctionGroups : Bool = false
     static var isGroupLeader : Bool? 
    // MARK: - TAB
    ...
    static func getCurrentColor() -> UIColor {
         return UIColor(
            red: CGFloat((0x6759A6 & 0xFF0000) >> 16) / 255.0,
            green: CGFloat((0x6759A6 & 0x00FF00) >> 8) / 255.0,
            blue: CGFloat(0x6759A6 & 0x0000FF) / 255.0,
            alpha: CGFloat(1.0)
        )
    }
    static var currentGroup: TourGroup?
    static var arraySelected: [Bool] = [true, false , false, false, false, false, false]
