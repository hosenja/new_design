...
open class SearchTextField: UITextField {
    ...
    open func filterPlaces(places: [GMSAutocompletePrediction]) {
        var items = [SearchTextFieldItem]()
        
        for value in places {
            items.append(SearchTextFieldItem(title: "\(value.attributedPrimaryText.string), \((value.attributedSecondaryText?.string)!)", place_id: value.placeID))
        }
    ...
    fileprivate func buildSearchTableView() {
        if let tableView = tableView, let shadowView = shadowView {
            ...
            resultsListHeader?.frame.width = UIScreen.main.bounds.width
            resultsListHeader?.frame.x = 0
...
extension SearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       // cell.detailTextLabel?.textColor = theme.subtitleFontColor
        cell.googlePlaceText.text = filteredResults[(indexPath as NSIndexPath).row].title
...
public struct SearchTextFieldTheme {
    ...
    public static func darkTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 40, bgColor: UIColor.white, borderColor: UIColor.white, separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 14), fontColor: UIColor(netHex: AppSetting.Color.blue))
    }
}
...
open class SearchTextFieldItem {
    ...
    public init(title: String, subtitle: String?, image: UIImage?, place_id: String?) {
        self.title = title
    ...
    public init(title: String, place_id: String?) {
        self.title = title
        self.place_id = place_id
    }
